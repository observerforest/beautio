...
export function ProductEditorDialog({
  ...
  const [progress, setProgress] = useState(t("保存前不会上传图片或修改数据库。"));
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>): void => {
    ...
    if (
      ...
      setError(locale === "en" ? `Product alias is limited to ${productAliasMaximumLength} characters.` : `产品别名最多 ${productAliasMaximumLength} 个字符，请缩短后再保存。`);
  ...
  const preview = clearImage ? (
    <ImagePlaceholder copy={t("保存后将清空管理图片")} />
  ) : localPreview !== null ? (
    <img src={localPreview} alt={`${selectedImage?.name ?? t("新图片")} ${t("预览")}`} className="h-full w-full object-cover" />
  ) : currentImage.status === "ready" ? (
    <img src={currentImage.src} alt={`${product.name} · ${t("当前管理图片")}`} className="h-full w-full object-cover" onError={currentImage.reportError} />
  ) : (
    <ImagePlaceholder copy={t(currentImage.status === "loading" ? "正在读取当前图片" : currentImage.status === "error" ? "当前图片读取失败" : product.image_ref === null ? "暂无管理图片" : "旧图片引用不自动加载")} />
  );

  const footer = (requestClose: () => void) => (
    <EditorFooter
      shared
      notice={t("修改将同步影响该产品的所有库存瓶")}
      progress={progress}
      formId={formId}
      saveLabel={t("保存产品资料")}
      busy={busy}
        ...
        <fieldset disabled={busy || writeCompleted} className="space-y-5 disabled:opacity-70">
          <div className="flex flex-col items-center gap-3">
            ...
            <button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 rounded-full border border-[#D8D4D1] px-4 py-2 text-xs text-[#7A7572] hover:bg-stone-50">
              <Icon name="camera" className="size-4" />{t("更换图片")}
            </button>
            <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={handleFile} />
            <p className="text-center text-[10px] text-[#A8A3A0]">{t("选择后只在点击保存时上传 JPEG、PNG 或静态 WebP。")}</p>
          ...
          <div className="space-y-4">
            <Field label={t("产品名称")} hint={t("必填；不会自动按名称查重或合并。")}>
              <input ref={nameRef} value={name} onChange={(event) => setName(event.target.value)} maxLength={200} className={editorInputClass} required />
            </Field>
            <Field
              label={t("产品别名")}
              hint={locale === "en" ? `A familiar or confirmed online name; up to ${productAliasMaximumLength} characters.` : `用户常用称呼或已确认的网络称呼；最多 ${productAliasMaximumLength} 个字符。`}
            ...
            <Field label={t("共享备注")} hint={t("同一 Product 的所有库存都会看到。")} counter={textCharacterCountLabel(sharedNotes, NOTES_MAXIMUM)}>
